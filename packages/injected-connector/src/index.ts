...
export class InjectedConnector extends AbstractConnector {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    ...
    // try to activate + get account via platon_requestAccounts
    let account
    try {
      account = await (window.alaya.send as Send)('platon_requestAccounts').then(
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.alaya.send as Send)('platon_accounts').then(sendReturn => parseSendReturn(sendReturn)[0])
    } catch {
    ...
    if (!account) {
      account = parseSendReturn((window.alaya.send as SendOld)({ method: 'platon_accounts' }))[0]
    }
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.alaya.send as Send)('platon_accounts').then(sendReturn => {
